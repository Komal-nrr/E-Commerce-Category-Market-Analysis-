# E-commerce Category Data Analysis

An end-to-end analysis of an e-commerce database, built with Python and MySQL.

The goal was simple: take raw transactional data sitting in a database and turn it into something you can actually make decisions from. Who's buying, what they're buying, when sales pick up, and which products carry the business.

## What's in here

The analysis is written as SQL queries run against the database, with Pandas handling the results and Matplotlib/Seaborn doing the plotting. Questions covered include:

- **Customer demographics** — where customers are located, how many are one-time vs. repeat buyers
- **Order trends** — order volume over time, seasonality, month-over-month growth
- **Sales performance** — revenue breakdowns by category, region, and time period
- **Product popularity** — top and bottom sellers, category-level comparisons

Queries range from basic aggregations to window functions and CTEs for things like running totals and customer retention rates.

## Tech stack

| Tool | Used for |
|------|----------|
| MySQL | Storing the data, writing the analytical queries |
| Python | Connecting to the DB, processing results |
| Pandas | Cleaning and reshaping query output |
